...
class SwinTransformer3D(nn.Module):
    ...
    def proj_out(self, x):
        ...
        norm_layer = nn.LayerNorm(ch).to(device=x.device)
        x = norm_layer(x)
    ...
    def forward(self, x):
        ...
        print('After patch_embed: {}'.format(x0.shape))

        x0 = self.pos_drop(x0)
        print('After pos drop: {}'.format(x0.shape))

        x0_out = self.proj_out(x0)
        x1 = self.layers1[0](x0.contiguous())
        print(' layer 1 contiguous: {}'.format(x0.shape))
        print('After layer 1: {}'.format(x1.shape))
